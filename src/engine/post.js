...
export function createPost(engine) {
  ...
  const P = {
    co: 0, fear: 0, blackout: 0, exposure: 1.0, bloom: 0.55, grain: 0.055, time: 0, saturation: 1.0, contrast: 1.04,
  ...
  const compMat = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uGrain: { value: 0.05 }, uCO: { value: 0 }, uFear: { value: 0 }, uBlack: { value: 0 }, uRes: { value: new THREE.Vector2() },
    ...
    fragmentShader: `uniform sampler2D tScene; uniform sampler2D tBloom; uniform float uBloom; uniform float uExp; uniform float uTime;
      ...
      void main(){
        ...
        float ca = 0.0012 + r2 * 0.004 + uCO * 0.004 + uFear * 0.002;
        vec3 c;
        ...
        c = mix(vec3(l), c, uSat * (1.0 - uCO * 0.45));
        c = (c - 0.5) * uCon + 0.5;
        float vig = smoothstep(0.95, 0.25 - uFear * 0.2, sqrt(r2) * (1.0 + uFear * 0.6));
        c *= mix(0.55, 1.0, vig);
  ...
  return {
    ...
    render(scene, camera, dt) {
      ...
      u.uGrain.value = P.grain; u.uCO.value = P.co; u.uFear.value = P.fear; u.uBlack.value = P.blackout; u.uSat.value = P.saturation; u.uCon.value = P.contrast;
      pass(compMat, null);
    },
    ...
    compositeTo(srcTarget, outTarget) {
      const u = compMat.uniforms; const save = { b: u.uBloom.value, co: u.uCO.value, f: u.uFear.value, bl: u.uBlack.value, g: u.uGrain.value };
      ...
      u.uBloom.value = save.b; u.uCO.value = save.co; u.uFear.value = save.f; u.uBlack.value = save.bl; u.uGrain.value = save.g;
    },
  };
}
