...
import { clamp, lerp, smooth, dist, dist2d, lerp3 } from './util.js?v=a148af98'
import { LINES } from './content/story.js?v=a148af98'
...
export const DIR = {
  ...
  dreadGap: [[100, 190], [70, 140]],   // s between dread events on night 1 / night 2 (night 3+: x0.85 a night)
...
export const EVENTS = [
  ...
  { id: 'lure', fam: 'hallu', tier: 2, max: 1, after: [90, 140],
    ...
    go: (c, D) => {   // a light out over the dark, flashing SOS, too evenly. Go after it and it backs off the trail; blink,
      ...
      D.spike(0.2, 'light')
      let life = 150, walked = 0, last = null, stage = 0
      D.frame((dt, c2) => {
        life -= dt
...
export class Director {
  constructor({ rng = Math.random } = {}) {
    ...
    this.ev = null                // the event in progress (one at a time)
    this.watchers = []            // background checks outliving their event (the moved clock)
    this.period = null; this.count = {}; this.nTonight = 0
    this.t = 0; this.gapUntil = 0; this.nextDread = 1e9; this.nextHallu = 0; this.nextSleepy = 0
    this.lastId = null; this.lastFam = null; this.ownQuiet = 0; this.forced = null
  ...
  awayFromCab(c) { return !c.inCab && this.cabLeftAt != null && c.t - this.cabLeftAt >= 45 && (c.eye[1] < 25 || dist2d(c.eye, this.W.cab) > 8) }
    ...
    const hush = after != null ? after : this.r(def.after[0], def.after[1])
    this.gapUntil = Math.max(this.gapUntil, t + hush, ev.t0 + DIR.minGap)
  ...
  update(dt, ctx, actions = {}) {
    ...
    if (hold) {   // freeze: every clock, every pending step (clean-ups still run), the eyelids
      this.gapUntil += dt; this.nextDread += dt; this.nextHallu += dt; this.nextSleepy += dt
    ...
    for (let guard = 0; guard < 64; guard++) {
      ...
      this.ev = prev && !prev.over ? prev : this.ev && !this.ev.over ? this.ev : null
    }
    if (!hold) {
      if (this.ev && this.ev.frame) { try { this.ev.frame(dt, c) } catch (err) { try { console.warn('director: event failed', err) } catch (e) { /* ok */ } this.done() } }
      this.watchers = this.watchers.filter((w) => t < w.until && !w.fn(c))
      if (this.ev && t - this.ev.t0 > 240) this.done()   // nothing lasts this long: a safety net
    }
    this.body(dt, c, hold)
    ...
    if (t < this.gapUntil) return
    const F = c.fatigue
    // the body first: a collapse doesn't wait its turn
    if (F && c.level >= DIR.collapseAt && !c.chase) this.collapseT += dt; else this.collapseT = Math.max(0, this.collapseT - dt * 2)
    if (this.collapseT >= DIR.collapseHold && this.start(BY_ID.collapse, c)) { this.collapseT = 0; return }
    ...
    const due = []
    if (sleepy && t >= this.nextSleepy) due.push('sleepy')
    if (lhOK && t >= this.nextHallu) due.push('hallu')
    if (c.night && t >= this.nextDread && !c.chase && c.bear < 0.3) due.push('dread')
    if (!due.length) return
    if (due.length > 1 && due.includes(this.lastFam)) due.push(due.splice(due.indexOf(this.lastFam), 1)[0])   // take turns
  ...
  pick(fam, c) {
    ...
    for (const e of EVENTS) {
      if (e.fam !== fam || e.id === this.lastId || e.id === 'collapse') continue
  ...
  start(e, c) {
    ...
    if (this.ev === ev && ev.over) this.ev = null
    this.count[e.id] = (this.count[e.id] || 0) + 1; if (e.fam === 'dread') this.nTonight++
    this.lastId = e.id; this.lastFam = e.fam
    this.gapUntil = Math.max(this.gapUntil, this.t + DIR.minGap)
  ...
  clear(actions = this.A) {
    ...
    if (this.ev) { for (const h of this.ev.phantoms) { try { h.remove() } catch (e) { /* gone */ } } this.ev = null }
    this.q = []; this.watchers = []; this.lidEv = []; this.lidSym = []
    if (this.lidOut > 0) { this.act('lids', 0); this.lidOut = 0 }
    this.period = null; this.gapUntil = 0; this.collapseT = 0; this.forced = null
  }
