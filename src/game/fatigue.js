...
export class Fatigue {
  ...
  tick(dtH, ctx = {}) {
    ...
    // warnings, once each (they come back after you've slept below them); never while you're in bed
    const L = this.level
    if (!ctx.resting) FAT.warn.forEach((w, i) => { const k = 'tired' + (i + 1); if (L >= w && !this.warned[k]) { this.warned[k] = true; if (!FAT.warn.some((w2, j) => j > i && L >= w2)) ev.push(k) } else if (L < w - 0.15) this.warned[k] = false })
    return ev
  }
